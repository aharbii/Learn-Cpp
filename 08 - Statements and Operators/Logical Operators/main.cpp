...
int main() 
{
    ...
    // with_bounds = (num > lower) && (num < upper);
    // cout << num << " is between " << lower << " and " << upper << " : " << with_bounds << endl;

    // Determine if an entered intger is outside two other integers
    // assume lower < upper
    ...
    // Determine if an entered intger is exactly on the boundary
    // assume lower < upper
    ...
    // cout << num << " is ont he bounds which are " << lower << " and " << upper << " : " << on_bounds << endl;
    ...
    const int wind_speed_for_coat{25}; // wind over this valude requires a coat
